...
/**
 * @swagger
 * /api/v1/phim/quoc-gia/{slug}:
 *   get:
 *     description: Trả ra danh sách phim theo quốc gia
 *     parameters:
 *       - in: path
 *         name: slug
 *         required: true
 ...
 *     responses:
 *       200:
 *         description: Danh sách phim theo quốc gia
 */
export default async function handler(
  ...
  const schema = z.object({
    ...
    slug: z
      ...
      .refine(
        async (slug) => {
          const country = await Country.findOne({ slug }).select("_id name");
          return !!country;
        },
        {
