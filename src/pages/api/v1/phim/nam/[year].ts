...
/**
 * @swagger
 * /api/v1/phim/nam/{year}:
 *   get:
 *     description: Trả ra danh sách phim theo năm
 *     parameters:
 *       - in: path
 *         name: year
 *         required: true
 ...
 *     responses:
 *       200:
 *         description: Danh sách phim theo năm
 */
export default async function handler(
  ...
  const schema = z.object({
    ...
    year: z
      ...
      .refine(
        async (year) => {
          const movie = await Movie.findOne({ year }).select("_id name");
          return !!movie;
        },
        {
          message: "Year does not exist in movies collection",
        }
      ),
  });
